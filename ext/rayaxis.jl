...
const Point2d = Point2{Float32}
struct Spines
  top::Vector{Point2d}
  bottom::Vector{Point2d}
  left::Vector{Point2d}
  right::Vector{Point2d}
end
Spines()= Spines(Point2d[], Point2d[], Point2d[], Point2d[])

function intersect_rect(rect::Rect2d, line_start::Point2d, line_end::Point2d)
...
function valid_line_in_limits(trans, trans_rev, rect, point_start, point_stop, n=100)
    ...
    for i in 1:n
        ...
        # transform from xy to the axis system
...
@inline function __ecef2el(θ,h,majoraxis,minoraxis,e², b²)
  N₀(θ) = majoraxis * sqrt(1 - e² * sind(θ)^2)
  return Point2d(
    (N₀(θ) + h) * cosd(θ),
    (N₀(θ) * b² + h) * sind(θ)
  )
  ...
  __transform_func(el::Ellipsoid)=(θ,h)->__ecef2el(θ,h,el.a,el.b,el.e2,1-el.e2)
el=Ellipsoid(;a="1000",b="800")
el.a
function Makie.initialize_block!(axis::RayAxis)


    # Create the ellipsoid for the transformation.
    majoraxis = string(axis.majoraxis)
    minoraxis = string(axis.minoraxis)
    el = Ellipsoid(a=majoraxis, b=minoraxis)
   ...
   __transform_func(el::Ellipsoid)=(θ,h)->__ecef2el(θ,h,el.a,el.b,el.e2,1-el.e2)
    ...
    # and linked to those.
    transform_obs = Observable{Any}(__transform_func(el); ignore_equal_values=true)
    transform_ticks_obs = Observable{Any}(__transform_func(el); ignore_equal_values=true)
